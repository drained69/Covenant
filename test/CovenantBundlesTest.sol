...
contract CovenantBundlesTest is BaseTest {
    using UtilsLib for uint256;

    address internal constant PERMIT2 = 0x000000000022D473030F116dDEE9F6B43aC78BA3;

    CovenantBundles internal covenantBundles;
    ...
    function setUp() public override {
        ...
        covenantBundles = new CovenantBundles(address(covenant));
        assertEq(covenantBundles.COVENANT(), address(covenant));
        deployCodeTo("Permit2", PERMIT2);

        // Set settlement fees to max for all breakpoints.
        covenant.setFeeClaimer(makeAddr("feeClaimer"));
        for (uint256 i; i <= 6; i++) {
        ...
        offers[0].notary = address(dummyNotary);
        ...
        offers[1].notary = address(dummyNotary);
        ...
        vm.prank(borrower);
        covenant.setIsAuthorized(address(covenantBundles), true, borrower);
        vm.prank(borrower);
        covenant.setIsAuthorized(address(this), true, borrower);
        vm.prank(lender);
        covenant.setIsAuthorized(address(covenantBundles), true, lender);
        vm.prank(lender);
        covenant.setIsAuthorized(address(this), true, lender);

        vm.prank(lender);
        loanToken.approve(address(covenantBundles), type(uint256).max);
    }
        ...
        returns (TokenPermit memory)
    {
        bytes32 structHash = keccak256(
            abi.encode(ERC20Permit(token).PERMIT_TYPEHASH(), owner, address(covenantBundles), amount, nonce, deadline)
        );
    ...
    function testUnauthorized() public {
        ...
        takes[0] = Take({offer: offers[0], units: 100, notaryData: hex""});

        vm.prank(address(0xdead));
        vm.expectRevert(ICovenantBundles.Unauthorized.selector);
        covenantBundles.buyWithUnitsTargetAndWithdrawCollateral(
    ...
    function testSellUnitsTarget(uint256 offerUnits0, uint256 offerUnits1, uint256 units) public {
        ...
        takes[0] = Take({offer: offers[0], units: offerUnits0, notaryData: hex""});
        takes[1] = Take({offer: offers[1], units: offerUnits1, notaryData: hex""});

        if (offerUnits1 >= units - fromOffer0) {
            vm.prank(borrower);
            covenantBundles.supplyCollateralAndSellWithUnitsTarget(
                units, 0, borrower, borrower, new CollateralSupply[](0), takes, 0, address(0)
            );

            uint256 consumed0 = covenant.consumed(offers[0].maker, offers[0].group);
            uint256 consumed1 = covenant.consumed(offers[1].maker, offers[1].group);
            assertEq(consumed0, fromOffer0, "consumed offer 0");
            assertEq(consumed0 + consumed1, covenant.debtOf(id, borrower), "total consumed");
            assertEq(covenant.debtOf(id, borrower), units, "debt");
        } else {
            vm.prank(borrower);
            vm.expectRevert(ICovenantBundles.OutOfOffers.selector);
            covenantBundles.supplyCollateralAndSellWithUnitsTarget(
                units, 0, borrower, borrower, new CollateralSupply[](0), takes, 0, address(0)
            );
        }
    ...
    function testBuyBuyerAssetsTarget(uint256 offerUnits0, uint256 offerUnits1, uint256 targetBuyerAssets) public {
        ...
        if (offerUnits1 >= units - fromOffer0) {
            vm.prank(lender);
            covenantBundles.buyWithAssetsTargetAndWithdrawCollateral(
                targetBuyerAssets,
                0,
                lender,
                _noPermit(),
    ...
    function testBuyBuyerAssetsTargetPermit2() public {
        ...
        vm.prank(lender);
        loanToken.approve(address(covenantBundles), 0);
        ...
        Take[] memory takes = new Take[](1);
        takes[0] = Take({offer: offers[0], units: units, notaryData: hex""});
        ...
        assertEq(loanToken.allowance(lender, address(covenantBundles)), 0);
    ...
    function testBuyUnitsTargetInconsistentMarket() public {
        ...
        Take[] memory takes = new Take[](2);
        takes[0] = Take({offer: offers[0], units: 1, notaryData: hex""});
        takes[1] = Take({offer: offers[1], units: 1, notaryData: hex""});

        vm.prank(lender);
        vm.expectRevert(ICovenantBundles.InconsistentMarket.selector);
        covenantBundles.buyWithUnitsTargetAndWithdrawCollateral(
    ...
    function testSellSellerAssetsTarget(uint256 offerUnits0, uint256 offerUnits1, uint256 targetSellerAssets) public {
        ...
        uint256 neededFromOffer1;
        {
            ...
            uint256 sellerPrice = price - covenant.settlementFee(id, market.maturity - vm.getBlockTimestamp());
        ...
        if (offerUnits1 >= neededFromOffer1) {
            vm.prank(borrower);
            covenantBundles.supplyCollateralAndSellWithAssetsTarget(
                targetSellerAssets,
                type(uint256).max,
                borrower,
                borrower,
    ...
    function testBuyUnitsTargetWithReferralFee(uint256 units, uint256 referralFeePct) public {
        ...
        Take[] memory takes = new Take[](1);
        takes[0] = Take({offer: offers[0], units: type(uint256).max, notaryData: hex""});
        ...
        assertEq(covenant.debtOf(id, borrower), units, "units filled");
        ...
        assertEq(loanToken.balanceOf(address(covenantBundles)), 0, "bundler residual");
    }
    ...
    function testSellUnitsTargetWithReferralFee(uint256 units, uint256 referralFeePct) public {
        ...
        uint256 price = TickLib.tickToPrice(MAX_TICK);
        covenant.initMarket(market);
        uint256 _settlementFee = covenant.settlementFee(id, market.maturity - vm.getBlockTimestamp());
        ...
        assertEq(covenant.debtOf(id, borrower), units, "units sold");
    ...
    function testSellSellerAssetsTargetWithReferralFee(uint256 targetSellerAssets, uint256 referralFeePct) public {
        // Bound such that preFeeTarget = target * WAD / (WAD - pct) stays under the uint128 unit ceiling of Covenant.
    ...
    function testRepayWithReferralFee(uint256 units, uint256 assets, uint256 referralFeePct) public {
        ...
        sellTakes[0] = Take({offer: offers[0], units: units, notaryData: hex""});
        collateralize(market, borrower, units);
        vm.prank(borrower);
        covenantBundles.supplyCollateralAndSellWithUnitsTarget(
        ...
        vm.prank(borrower);
        loanToken.approve(address(covenantBundles), assets);

        vm.prank(borrower);
        covenantBundles.repayAndWithdrawCollateral(
            market, assets, borrower, _noPermit(), new CollateralWithdrawal[](0), address(0), referralFeePct, referrer
        );

        assertEq(covenant.debtOf(id, borrower), units - expectedUnits, "debt");
        assertEq(loanToken.balanceOf(referrer), expectedFee, "referrer fee");
    ...
    function testRepayWithReferralFeeFullDebtInversion(uint256 debt, uint256 referralFeePct) public {
        ...
        Take[] memory sellTakes = new Take[](1);
        sellTakes[0] = Take({offer: offers[0], units: debt, notaryData: hex""});
        ...
        assertEq(covenant.debtOf(id, borrower), 0, "debt fully repaid");
        assertEq(loanToken.balanceOf(referrer), expectedFee, "referrer fee");
        assertEq(loanToken.balanceOf(borrower), 0, "borrower spent assets");
        assertEq(loanToken.balanceOf(address(covenantBundles)), 0, "bundler residual");
    }
    ...
    function testPctExceeded() public {
        ...
        buyTakes[0] = Take({offer: offers[0], units: 1, notaryData: hex""});

        vm.startPrank(lender);
        vm.expectRevert(ICovenantBundles.PctExceeded.selector);
        covenantBundles.buyWithUnitsTargetAndWithdrawCollateral(
        ...
        returns (uint256[] memory amounts)
    {
        ...
        for (uint256 i; i < numCollaterals; i++) {
            ...
            vm.startPrank(taker);
            ERC20(market.collateralParams[i].token).approve(address(covenant), amounts[i]);
            covenant.supplyCollateral(market, i, amounts[i], taker);
            vm.stopPrank();
        }
    }
    ...
    function testBuyUnitsTargetWithCollateralWithdrawals(uint256 numCollaterals) public {
        ...
        for (uint256 i; i < numCollaterals; i++) {
            assertEq(covenant.collateral(id, lender, i), amounts[i] - amounts[i] / 4);
    ...
    function testSellUnitsTargetWithCollateralSupplies(uint256 numCollaterals) public {
        ...
        for (uint256 i; i < numCollaterals; i++) {
            ...
            vm.prank(borrower);
            ERC20(market.collateralParams[i].token).approve(address(covenantBundles), amount);
        ...
        for (uint256 i; i < numCollaterals; i++) {
            assertEq(covenant.collateral(id, borrower, i), supplies[i].assets);
        }
    ...
    function testSellUnitsTargetPermit2() public {
        ...
        vm.startPrank(borrower);
        ERC20(collateralToken).approve(address(covenantBundles), 0);
        ...
        assertEq(ERC20(collateralToken).allowance(borrower, address(covenantBundles)), 0);
        assertEq(ERC20(collateralToken).allowance(borrower, PERMIT2), 0);
        assertEq(covenant.collateral(id, borrower, 0), amount);
        assertEq(covenant.debtOf(id, borrower), units);
    }
    ...
    function testRepay(uint256 units, uint256 repayUnits, uint256 withdrawAssets) public {
        ...
        // Borrower sells units to get loan token + accumulate debt and collateral on Covenant.
        Take[] memory sellTakes = new Take[](1);
        sellTakes[0] = Take({offer: offers[0], units: units, notaryData: hex""});
        collateralize(market, borrower, units);
        uint256 collateralAmount = covenant.collateral(id, borrower, 0);
        vm.prank(borrower);
        covenantBundles.supplyCollateralAndSellWithUnitsTarget(
            units, 0, borrower, borrower, new CollateralSupply[](0), sellTakes, 0, address(0)
        );
        ...
        vm.prank(borrower);
        loanToken.approve(address(covenantBundles), repayUnits);
        ...
        assertEq(covenant.debtOf(id, borrower), units - repayUnits, "debt");
        assertEq(covenant.collateral(id, borrower, 0), collateralAmount - withdrawAssets, "remaining collateral");
        assertEq(
    ...
    function testSellSellerAssetsTargetWithCollateralSupplies(uint256 numCollaterals) public {
        deal(address(loanToken), address(covenantBundles), 0);
        numCollaterals = bound(numCollaterals, 1, 2);
        uint256 units = 100e18;
    ...
    function testSellUnitsTargetAveragePriceTooLow(uint256 tick) public {
        ...
        vm.prank(borrower);
        vm.expectRevert(ICovenantBundles.SellerAssetsTooLow.selector);
        covenantBundles.supplyCollateralAndSellWithUnitsTarget(
    ...
    function testBuyBuyerAssetsTargetAveragePriceExceeded(uint256 tick) public {
        ...
        vm.prank(lender);
        vm.expectRevert(ICovenantBundles.UnitsTooLow.selector);
        covenantBundles.buyWithAssetsTargetAndWithdrawCollateral(
    ...
    function testSellSellerAssetsTargetAveragePriceTooLow(uint256 tick) public {
        ...
        vm.prank(borrower);
        vm.expectRevert(ICovenantBundles.UnitsTooHigh.selector);
        covenantBundles.supplyCollateralAndSellWithAssetsTarget(
            targetSellerAssets, price + 1, borrower, borrower, new CollateralSupply[](0), takes, 0, address(0)
        );
    }

    // Partially consumed offers: _availableUnits caps the units forwarded to fillOffer().
    ...
    function testSellUnitsTargetPartiallyConsumed() public {
        ...
        vm.prank(lender);
        covenant.setConsumed(offers[0].group, 30, lender);

        Take[] memory takes = new Take[](2);
        takes[0] = Take({offer: offers[0], units: 100, notaryData: hex""});
        takes[1] = Take({offer: offers[1], units: 100, notaryData: hex""});
        ...
        assertEq(covenant.consumed(offers[0].maker, offers[0].group), 100, "consumed offer 0");
        assertEq(covenant.consumed(offers[1].maker, offers[1].group), 30, "consumed offer 1");
        assertEq(covenant.debtOf(id, borrower), 100, "debt");
    }
    ...
    function testSellSellerAssetsTargetPartiallyConsumed() public {
        ...
        assertEq(consumed0 - 30 + consumed1, covenant.debtOf(id, borrower), "total consumed");
        assertEq(loanToken.balanceOf(borrower), targetSellerAssets, "borrower balance");
    }
    ...
    function testBuyUnitsTargetPartiallyConsumed() public {
        ...
        vm.prank(borrower);
        covenant.setConsumed(offers[0].group, 30, borrower);
