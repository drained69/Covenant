...
contract OtherFunctionsTest is BaseTest {
        ...
        public
    {
        ...
        covenant.supplyCollateral(market, 0, additionalCollateral, borrower);
        withdraw = bound(withdraw, 0, additionalCollateral);
        uint256 initialCollateral = covenant.collateral(id, borrower, 0);

        vm.prank(borrower);
        covenant.withdrawCollateral(market, 0, withdraw, borrower, borrower);

        assertEq(covenant.collateral(id, borrower, 0), initialCollateral - withdraw, "collateral of");
        assertEq(
            ERC20(collateralToken).balanceOf(address(covenant)), initialCollateral - withdraw, "balance of covenant"
        );
        ...
        public
    {
        ...
        vm.prank(borrower);
        vm.expectRevert(ICovenant.UnhealthyBorrower.selector);
        covenant.withdrawCollateral(market, 0, withdraw, borrower, borrower);
    }
    ...
    function testRepay(uint256 units, uint256 repaid) public {
        ...
        vm.prank(borrower);
        covenant.repay(market, repaid, borrower, address(0), hex"");

        assertEq(covenant.debtOf(id, borrower), units - repaid);
        assertEq(covenant.withdrawable(id), repaid);
        assertEq(loanToken.balanceOf(address(covenant)), repaid);
    ...
    function testRepayCallback(uint256 units, uint256 repaid, bytes memory data, address caller) public {
        ...
        vm.prank(borrower);
        covenant.setIsAuthorized(caller, true, borrower);
        vm.prank(address(callback));
        loanToken.approve(address(covenant), repaid);

        vm.prank(caller);
        covenant.repay(market, repaid, borrower, address(callback), data);
    ...
    function testWithdraw(uint256 units, uint256 withdraw) public {
        ...
        vm.prank(lender);
        covenant.withdraw(market, withdraw, lender, lender);

        assertEq(covenant.creditOf(id, lender), units - withdraw, "creditOf");
        assertEq(covenant.withdrawable(id), 0, "withdrawable");
        assertEq(covenant.totalUnits(id), units - withdraw, "totalUnits");
        assertEq(loanToken.balanceOf(address(covenant)), 0, "balance of covenant");
    ...
    function testWithdrawToReceiver(uint256 units, uint256 withdraw) public {
        ...
        vm.prank(lender);
        covenant.withdraw(market, withdraw, lender, receiver);
    ...
    function testWithdrawCollateralToReceiver(uint256 supply, uint256 withdraw) public {
        ...
        covenant.supplyCollateral(market, 0, supply, address(this));

        covenant.withdrawCollateral(market, 0, withdraw, address(this), receiver);
    ...
    function testSetConsumed(address user, bytes32 group, uint256 amount) public {
        vm.prank(user);
        covenant.setConsumed(group, amount, user);
        assertEq(covenant.consumed(user, group), amount, "consumed");
    }
    ...
    function testSetConsumedIncreasing(address user, bytes32 group, uint256 amount0, uint256 amount1) public {
        ...
        vm.prank(user);
        covenant.setConsumed(group, amount0, user);
        assertEq(covenant.consumed(user, group), amount0, "consumed 0");

        vm.prank(user);
        covenant.setConsumed(group, amount1, user);
        assertEq(covenant.consumed(user, group), amount1, "consumed 1");
    }
    ...
    function testSetConsumedDecreasingReverts(address user, bytes32 group, uint256 amount0, uint256 amount1) public {
        ...
        vm.prank(user);
        vm.expectRevert(ICovenant.AlreadyConsumed.selector);
        covenant.setConsumed(group, amount1, user);
    }
    ...
    function testTouchMarket(Market memory _market) public {
        ...
        covenant.setDefaultContinuousFee(_market.loanToken, MAX_CONTINUOUS_FEE);
        for (uint256 i = 0; i < 7; i++) {
            covenant.setDefaultSettlementFee(_market.loanToken, i, maxSettlementFee(i));
        }

        bytes32 _id = covenant.initMarket(_market);
        assertEq(covenant.tickSpacing(_id) > 0, true, "market created");
        uint16[7] memory fees = covenant.settlementFeeCbps(_id);
        for (uint256 i = 0; i < 7; i++) {
            assertEq(fees[i], covenant.defaultSettlementFeeCbp(_market.loanToken, i), "fees");
            assertGt(fees[i], 0, "fee nonzero");
        }
        assertEq(covenant.continuousFee(_id), MAX_CONTINUOUS_FEE, "continuousFee");
    }
    ...
    function testToMarket(Market memory _market) public {
        ...
        bytes32 _id = covenant.initMarket(_market);
        Market memory marketFromId = covenant.toMarket(_id);
    ...
    function testToId(Market memory _market) public view {
        ...
        bytes32 actual = covenant.toId(_market);
    ...
    function testToIdStableAcrossHardfork(Market memory _market, Market memory otherMarket, uint64 newChainId) public {
        ...
        bytes32 idBefore = covenant.initMarket(_market);
        uint256 capturedChainId = covenant.INITIAL_CHAIN_ID();

        vm.chainId(newChainId);

        assertEq(covenant.INITIAL_CHAIN_ID(), capturedChainId, "INITIAL_CHAIN_ID changed");
        assertEq(covenant.toId(_market), idBefore, "toId changed");
        Market memory roundTrip = covenant.toMarket(idBefore);
        ...
        bytes32 otherId = covenant.initMarket(otherMarket);
        Market memory otherRoundTrip = covenant.toMarket(otherId);
    ...
    function testSupplyCollateralDoesNotCallOracle(uint256 collateral) public {
        ...
        covenant.supplyCollateral(marketWithRevertingOracle, 0, collateral, borrower);
    }
    ...
    function testWithdrawCollateralToZeroDoesNotCallOracle(uint256 collateral) public {
        ...
        assertEq(covenant.collateral(_id, borrower, 0), collateral, "collateral should be set");

        revertingOracle.stopOracle();

        vm.prank(borrower);
        covenant.withdrawCollateral(marketWithRevertingOracle, 0, collateral, borrower, borrower);
    }
    ...
    function testMaturityTooFar(uint256 maturity) public {
        ...
        vm.expectRevert(ICovenant.MaturityTooFar.selector);
        covenant.initMarket(longMarket);
    }
    ...
    function testZeroCollaterals() public {
        ...
        vm.expectRevert(ICovenant.NoCollateralParams.selector);
        covenant.initMarket(_market);
    }
    ...
    function testMaxCollaterals(uint256 numCollaterals) public {
        ...
        vm.expectRevert(ICovenant.TooManyCollateralParams.selector);
        covenant.initMarket(_market);
    }
    ...
    function testCollateralsNotSorted() public {
        ...
        _market.collateralParams = collateralParams;
        vm.expectRevert(ICovenant.CollateralParamsNotSorted.selector);
        covenant.initMarket(_market);
    }
    ...
    function testLltvNotAllowedAboveWad(uint256 lltv) public {
        ...
        _market.collateralParams = collateralParams;
        vm.expectRevert(ICovenant.LltvNotAllowed.selector);
        covenant.initMarket(_market);
    }
    ...
    function testMaxCollateralsPerBorrower() public {
        ...
        for (uint256 i = 0; i < MAX_COLLATERALS_PER_BORROWER; i++) {
            ...
            ERC20(token).approve(address(covenant), 1e18);
            covenant.supplyCollateral(_market, i, 1e18, borrower);
        }

        address lastToken = _market.collateralParams[numCollaterals - 1].token;
        deal(lastToken, address(this), 1e18);
        ERC20(lastToken).approve(address(covenant), 1e18);
        vm.expectRevert(ICovenant.TooManyActivatedCollaterals.selector);
        covenant.supplyCollateral(_market, numCollaterals - 1, 1e18, borrower);
    }
    ...
    function testCollateralBitmapCtzSingleCollateral(uint256 collateralIndex) public {
        ...
        deal(token, address(this), 1e18);
        ERC20(token).approve(address(covenant), 1e18);
        covenant.supplyCollateral(_market, collateralIndex, 1e18, borrower);

        uint128 collateralBitmap = covenant.collateralBitmap(toId(_market), borrower);
    ...
    function testCollateralBitmapCountBitsAfterMultipleSupplies(uint256 k) public {
        ...
        uint128 collateralBitmap = covenant.collateralBitmap(_id, borrower);
    ...
    function testCollateralBitmapClearedOnFullWithdraw(uint256 collateralIndex) public {
        ...
        bytes32 _id = toId(_market);
        assertEq(UtilsLib.countBits(covenant.collateralBitmap(_id, borrower)), numCollaterals, "all bits set");

        // Withdraw one collateral fully.
        vm.prank(borrower);
        covenant.withdrawCollateral(_market, collateralIndex, 1e18, borrower, borrower);
    ...
    function testCollateralBitmapClearedOnFullLiquidation(uint256 collateralIndex) public {
        ...
        covenant.seize(_market, collateralIndex, 1e18, 0, borrower, true, address(this), address(0), "");
    ...
    function testInvalidLif(uint256 lif) public {
        ...
        vm.expectRevert(ICovenant.InvalidMaxLif.selector);
        covenant.initMarket(_market);
    }
    ...
    function testValidLifCursor025() public {
        ...
        covenant.initMarket(_market);
        assertEq(covenant.tickSpacing(toId(_market)) > 0, true, "market created with cursor 0.25");
    }
    ...
    function testValidLifCursor05() public {
        ...
        covenant.initMarket(_market);
        assertEq(covenant.tickSpacing(toId(_market)) > 0, true, "market created with cursor 0.5");
    }
    ...
    function testMarketStateGetter(Market memory _market, uint256 _defaultContinuousFee) public {
        ...
        covenant.setDefaultContinuousFee(_market.loanToken, _defaultContinuousFee);
        for (uint256 i = 0; i < 7; i++) {
            covenant.setDefaultSettlementFee(_market.loanToken, i, maxSettlementFee(i));
        }
        ...
        assertEq(settlementFeeCbp0, covenant.defaultSettlementFeeCbp(_market.loanToken, 0), "settlementFeeCbp0");
        assertEq(settlementFeeCbp1, covenant.defaultSettlementFeeCbp(_market.loanToken, 1), "settlementFeeCbp1");
        assertEq(settlementFeeCbp2, covenant.defaultSettlementFeeCbp(_market.loanToken, 2), "settlementFeeCbp2");
        assertEq(settlementFeeCbp3, covenant.defaultSettlementFeeCbp(_market.loanToken, 3), "settlementFeeCbp3");
        assertEq(settlementFeeCbp4, covenant.defaultSettlementFeeCbp(_market.loanToken, 4), "settlementFeeCbp4");
        assertEq(settlementFeeCbp5, covenant.defaultSettlementFeeCbp(_market.loanToken, 5), "settlementFeeCbp5");
        assertEq(settlementFeeCbp6, covenant.defaultSettlementFeeCbp(_market.loanToken, 6), "settlementFeeCbp6");
    }
    ...
    function testMarketStateAfterTake() public {
        ...
        (uint128 totalUnits,,,,,,,,,,, uint32 _continuousFee, uint8 tickSpacing) = covenant.marketState(id);

        assertEq(totalUnits, units, "totalUnits after fillOffer");
        assertEq(_continuousFee, MAX_CONTINUOUS_FEE, "continuousFee after fillOffer");
        assertEq(tickSpacing, 4, "tickSpacing after fillOffer");
    }

    function testCovenantRevertsOnCallbacks(address msgSender, bytes calldata data) public {
        ...
        for (uint256 i = 0; i < selectors.length; i++) {
            vm.prank(msgSender);
            (bool success,) = address(covenant).call(abi.encodePacked(selectors[i], data));
            assertFalse(success);
        }
    }
}
...
contract RepayCallback {
    ...
    function repay(Covenant covenant, Market memory market, uint256 units, address onBehalf, bytes memory data)
        external
    {
        ERC20(market.loanToken).approve(address(covenant), units);
        covenant.repay(market, units, onBehalf, address(this), data);
    }
