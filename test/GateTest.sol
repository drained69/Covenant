...
contract GateTest is BaseTest {
    ...
    function testEnterGateAllowsWhitelistedUsers(uint256 units) public {
        ...
        assertGt(covenant.creditOf(gatedId, lender), 0, "lender should have credit");
        assertGt(covenant.debtOf(gatedId, borrower), 0, "borrower should have debt");
    }
    ...
    function testEnterGateAllowsTakeWhenLenderHadCreditBefore(uint256 units) public {
        ...
        assertGt(covenant.creditOf(gatedId, lender), 0, "lender should already have credit");

        gate.setWhitelisted(lender, false);
        gate.setWhitelisted(borrower, false);

        fillOffer(0, otherBorrower, lenderOffer);
    }
    ...
    function testEnterGateAllowsTakeWhenBorrowerHadDebtBefore(uint256 units) public {
        ...
        assertGt(covenant.debtOf(gatedId, borrower), 0, "borrower should already have debt");

        gate.setWhitelisted(lender, false);
        gate.setWhitelisted(borrower, false);

        fillOffer(0, otherLender, borrowerOffer);
    }
    ...
    function testNoEnterGateCheckWhenBorrowerIsExitingBorrower(uint256 units) public {
        ...
        assertEq(covenant.debtOf(gatedId, borrower), 0, "borrower should have exited debt");
    }
    ...
    function testNoGateCheckWhenBothExit(uint256 units) public {
        ...
        exitOffer.notary = address(dummyNotary);
        exitOffer.expiry = vm.getBlockTimestamp() + 200;
        exitOffer.tick = MAX_TICK;

        deal(address(loanToken), otherBorrower, units);
        fillOffer(units, otherBorrower, exitOffer);

        assertEq(covenant.debtOf(gatedId, otherBorrower), 0, "otherBorrower should have exited");
    }
    ...
    function testNoGateCheckOnRepay(uint256 units) public {
        ...
        vm.prank(borrower);
        covenant.repay(gatedMarket, units, borrower, address(0), hex"");

        assertEq(covenant.debtOf(gatedId, borrower), 0, "borrower should have repaid");
    }
    ...
    function testNoGateCheckOnWithdraw(uint256 units) public {
        ...
        vm.prank(lender);
        covenant.withdraw(gatedMarket, units, lender, lender);

        assertEq(covenant.creditOf(gatedId, lender), 0, "lender should have withdrawn");
    }
    ...
    function testLiquidatorGateOnLiquidation(uint256 units, bool isWhitelisted) public {
        ...
        vm.prank(liquidator);
        if (!isWhitelisted) vm.expectRevert(ICovenant.SeizerIneligible.selector);
        covenant.seize(gatedMarket, 0, 1, 0, borrower, false, address(this), address(0), "");
    }
    ...
    function testLiquidatorGateOnBadDebt(uint256 units, bool isWhitelisted) public {
        ...
        vm.prank(liquidator);
        if (!isWhitelisted) vm.expectRevert(ICovenant.SeizerIneligible.selector);
        covenant.seize(gatedMarket, 0, 0, 0, borrower, false, address(this), address(0), "");
    }
    ...
    function testNoGateMeansUnrestricted(uint256 units) public {
        ...
        ungatedLenderOffer.notary = address(dummyNotary);
        ungatedLenderOffer.expiry = vm.getBlockTimestamp() + 200;
        ungatedLenderOffer.tick = MAX_TICK;

        fillOffer(units, borrower, ungatedLenderOffer);

        bytes32 ungatedId = toId(market);
        assertGt(covenant.debtOf(ungatedId, borrower), 0);
    }
