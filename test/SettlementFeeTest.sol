...
// The maximum debt from a fillOffer must fit in uint128, and the required collateral (debt / lltv)
...
contract SettlementFeeTest is BaseTest {
    ...
    function testBuyUnits(uint256 settlementFee, uint256 sellerTick, uint256 units) public {
        ...
        covenant.setDefaultSettlementFee(address(loanToken), 1, settlementFee);
        ...
        uint256 balanceBefore = loanToken.balanceOf(address(covenant));
        collateralize(market, borrower, MAX_DEBT);
        fillOffer(units, lender, borrowerOffer);

        assertEq(covenant.claimableSettlementFee(address(loanToken)), expectedFee, "claimable settlement fee");
        assertEq(loanToken.balanceOf(address(covenant)) - balanceBefore, expectedFee, "contract balance increase");
    }
    ...
    function testSevenDayTtmSettlementFee(
        ...
        // Must be set before initMarket, which snapshots defaultFees at creation time.
        covenant.setDefaultSettlementFee(address(loanToken), 1, settlementFee1Day);
        covenant.setDefaultSettlementFee(address(loanToken), 2, settlementFee7Days);
    ...
    function testPostMaturitySettlementFee(
        ...
        covenant.setDefaultSettlementFee(address(loanToken), 0, settlementFee0Day);
        borrowerOffer.tick = sellerTick;
        ...
        public
    {
        ...
        covenant.setDefaultSettlementFee(address(loanToken), 6, settlementFee360Days);
        borrowerOffer.tick = sellerTick;
    ...
    function testClaimSettlementFee(uint256 settlementFee, uint256 units, uint256 withdrawAmount) public {
        ...
        uint256 fee = covenant.claimableSettlementFee(address(loanToken));
        vm.assume(fee > 0);
        withdrawAmount = bound(withdrawAmount, 1, fee);
        address receiver = makeAddr("receiver");

        vm.prank(feeClaimer);
        covenant.claimSettlementFee(address(loanToken), withdrawAmount, receiver);

        assertEq(loanToken.balanceOf(receiver), withdrawAmount, "receiver balance");
        assertEq(covenant.claimableSettlementFee(address(loanToken)), fee - withdrawAmount, "remaining fee");
    }
    ...
    function testClaimSettlementFeeOnlyFeeClaimer(address caller) public {
        ...
        covenant.claimSettlementFee(address(loanToken), 0, caller);
    }
    ...
    function testClaimSettlementFeeExcessReverts() public {
        ...
        collateralize(market, borrower, MAX_DEBT);
        fillOffer(1000, lender, borrowerOffer);

        uint256 fee = covenant.claimableSettlementFee(address(loanToken));

        vm.prank(feeClaimer);
        vm.expectRevert();
        covenant.claimSettlementFee(address(loanToken), fee + 1, feeClaimer);
    }
    ...
    function testSettlementFeesAccumulate() public {
        ...
        collateralize(market, borrower, MAX_DEBT);
        fillOffer(1000, lender, borrowerOffer);
        uint256 feeAfterFirst = covenant.claimableSettlementFee(address(loanToken));

        borrowerOffer.group = keccak256("g2");
        fillOffer(1000, lender, borrowerOffer);
        uint256 feeAfterSecond = covenant.claimableSettlementFee(address(loanToken));

        assertEq(feeAfterSecond, feeAfterFirst * 2, "fees accumulated");
        assertEq(loanToken.balanceOf(address(covenant)) - balanceBefore, feeAfterSecond, "contract balance increase");
    }
}
