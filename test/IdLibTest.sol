...
import {Market} from "../src/interfaces/ICovenant.sol";
...
contract IdLibTest is Test {
    function testToIdIsInjectiveInMarket(
        ...
        bytes32 id1 = IdLib.toId(market1, chainid, covenant);
        bytes32 id2 = IdLib.toId(market2, chainid, covenant);
        assertNotEq(id1, id2);
    }

    function testToIdIsInjectiveInChainId(Market memory market, uint256 chainid1, uint256 chainid2, address covenant)
        public
        pure
    {
        vm.assume(chainid1 != chainid2);
        bytes32 id1 = IdLib.toId(market, chainid1, covenant);
        bytes32 id2 = IdLib.toId(market, chainid2, covenant);
        assertNotEq(id1, id2);
    }

    function testToIdIsInjectiveInCovenant(
        Market memory market,
        uint256 chainid,
        address covenantOne,
        address covenantTwo
    ) public pure {
        vm.assume(covenantOne != covenantTwo);
        bytes32 id1 = IdLib.toId(market, chainid, covenantOne);
        bytes32 id2 = IdLib.toId(market, chainid, covenantTwo);
        assertNotEq(id1, id2);
    }
}
