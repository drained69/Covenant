...
contract FlashLoanTest is BaseTest, IFlashLoanCallback {
        ...
        public
    {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            deal(tokens[i], address(covenant), amounts[i]);
        }

        vm.prank(caller);
        covenant.flashLoan(tokens, amounts, address(this), data);
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            assertEq(ERC20(tokens[i]).balanceOf(address(covenant)), amounts[i], "balanceOf(covenant)");
        }
