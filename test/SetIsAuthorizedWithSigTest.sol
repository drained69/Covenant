...
contract EcrecoverAuthorizerTest is BaseTest {
    ...
    function testEcrecoverAuthorizer() public {
        vm.prank(borrower);
        covenant.setIsAuthorized(address(ecrecoverAuthorizer), true, borrower);
        ...
        assertEq(covenant.isAuthorized(borrower, lender), true);
        ...
        assertEq(covenant.isAuthorized(borrower, lender), false);
    ...
    function testEcrecoverAuthorizerNonce(uint8 n) public {
        ...
        for (uint8 i = 0; i < n; i++) {
            ...
            assertEq(covenant.isAuthorized(borrower, lender), isAuth);
        }
    }
}
