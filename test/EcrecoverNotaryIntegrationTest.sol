...
/// @dev Tests covering the merkle/signature flow of `EcrecoverNotary` end-to-end via `Covenant.fillOffer`.
/// `EcrecoverNotaryTest` covers the notary in isolation; this file pins the integration with Covenant.
contract EcrecoverNotaryIntegrationTest is BaseTest {
    ...
    function setUp() public override {
        ...
        lenderOffer.notary = address(ecrecoverNotary);
    ...
    function merkleNotaryData(Offer[1] memory offers, address _signer) internal view returns (bytes memory) {
        bytes32 _root = root(offers);
        bytes32[] memory _proof = proof(offers);
        Signature memory _sig = signature(_root, privateKey[_signer], offers[0].notary, _proof.length);
        return abi.encode(_sig, _root, 0, _proof);
    }

    function merkleNotaryData(Offer[1] memory offers) internal view returns (bytes memory) {
        bytes32 _root = root(offers);
        bytes32[] memory _proof = proof(offers);
        Signature memory _sig = signature(_root, privateKey[offers[0].maker], offers[0].notary, _proof.length);
        return abi.encode(_sig, _root, 0, _proof);
    }

    /// @dev Builds merkle notary data with explicit root, leaf index, and proof — useful for negative tests where
    /// the signed root or the proof is intentionally inconsistent with the offer.
    function merkleNotaryData(Offer memory offer, bytes32 _root, uint256 _leafIndex, bytes32[] memory _proof)
        internal
        view
        returns (bytes memory)
    {
        Signature memory _sig = signature(_root, privateKey[offer.maker], offer.notary, _proof.length);
    ...
    function testTakeInvalidRoot(bytes32 invalidRoot) public {
        ...
        covenant.fillOffer(
            lenderOffer,
            merkleNotaryData(lenderOffer, invalidRoot, 0, new bytes32[](0)),
            100,
            borrower,
            borrower,
            address(0),
    ...
    function testTakeInvalidSignature() public {
        vm.expectRevert(IEcrecoverNotary.InvalidSignature.selector);
    ...
    function testTakeInvalidProofOneLeaf(bytes32[] memory _proof) public {
        ...
        covenant.fillOffer(
            lenderOffer,
            merkleNotaryData(lenderOffer, root([lenderOffer]), 0, _proof),
            100,
            borrower,
            borrower,
            address(0),
    ...
    function testTakeInvalidProof2LeavesWrongLeafHash(Offer memory otherOffer, bytes32[] memory _proof) public {
        ...
        covenant.fillOffer(
            lenderOffer,
            merkleNotaryData(lenderOffer, root([lenderOffer, otherOffer]), 0, _proof),
            100,
            borrower,
            borrower,
            address(0),
    ...
    function testTakeInvalidProof2LeavesWrongLeafIndex(Offer memory otherOffer) public {
        ...
        covenant.fillOffer(
            lenderOffer,
            merkleNotaryData(lenderOffer, root([lenderOffer, otherOffer]), 1, _proof),
            100,
            borrower,
            borrower,
            address(0),
    ...
    function testTakeTwoLeaves(uint256 units, Offer memory otherOffer) public {
        ...
        covenant.fillOffer(
            lenderOffer,
            merkleNotaryData(lenderOffer, root([lenderOffer, otherOffer]), 0, proof([lenderOffer, otherOffer])),
            units,
            borrower,
            borrower,
            address(0),
    ...
    function testTakeNotNotarized() public {
        vm.expectRevert();
        vm.prank(borrower);
        covenant.fillOffer(lenderOffer, emptySig, 100, borrower, borrower, address(0), hex"");
    }
    ...
    function testTakeOfferValidSignature(uint256 makerSecretKey, address sender) public {
        ...
        covenant.setIsAuthorized(address(ecrecoverNotary), true, vm.addr(makerSecretKey));
        vm.prank(sender);
        covenant.fillOffer(lenderOffer, merkleNotaryData([lenderOffer]), 0, sender, sender, address(0), hex"");
    }
    ...
    function testOfferAuthorization(uint256 makerSecretKey, address sender, uint256 otherSecretKey) public {
        ...
        covenant.fillOffer(
            lenderOffer,
            merkleNotaryData([lenderOffer], vm.addr(otherSecretKey)),
            100,
            sender,
            sender,
            address(0),
        ...
        public
    {
        ...
        covenant.setIsAuthorized(vm.addr(otherSecretKey), true, lenderOffer.maker);
        vm.prank(sender);
        covenant.fillOffer(
            lenderOffer,
            merkleNotaryData([lenderOffer], vm.addr(otherSecretKey)),
