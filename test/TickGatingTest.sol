...
/// @dev Integration tests for tick spacing enforcement in fillOffer() and spacing governance.
contract TickGatingTest is BaseTest {
    ...
    function testDefaultSpacingApplied() public view {
        assertEq(covenant.tickSpacing(id), 4, "market should inherit default spacing 4");
    }

    // --- Tick gating in fillOffer() ---
    ...
    function testTakeRevertsAtInaccessibleTick() public {
        ...
        vm.prank(borrower);
        vm.expectRevert(ICovenant.TickNotAccessible.selector);
        covenant.fillOffer(offer, hex"", units, borrower, borrower, address(0), hex"");
    }

    function testTakeRevertsAtSpacing2InaccessibleTick() public {
        // Refine to spacing 2.
        covenant.setMarketTickSpacing(id, 2);
    ...
    function testSetMarketTickSpacingOnlyTickSpacingSetter() public {
        vm.prank(lender);
        vm.expectRevert(ICovenant.OnlyTickSpacingSetter.selector);
        covenant.setMarketTickSpacing(id, 2);
    }

    function testSetMarketTickSpacingInvalid() public {
        vm.expectRevert(ICovenant.InvalidTickSpacing.selector);
        covenant.setMarketTickSpacing(id, 3);

        vm.expectRevert(ICovenant.InvalidTickSpacing.selector);
        covenant.setMarketTickSpacing(id, 0);
    ...
    function testSetMarketTickSpacingRequiresCreated() public {
        ...
        covenant.setMarketTickSpacing(bytes32(uint256(42)), 1);
    }
    ...
    function testSetTickSpacingSetterOnlyOwner() public {
        ...
        covenant.setTickSpacingSetter(lender);
    }
    ...
    function testCoarserTicksStillValidAfterRefinement(uint256 tick) public {
        ...
        deal(address(loanToken), lender, units.mulDivUp(price, WAD));
        collateralize(market, borrower, units);
        fillOffer(units, borrower, offer2);

        assertEq(covenant.creditOf(id, lender), 2 * units);
    }
}
