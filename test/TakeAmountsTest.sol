...
contract TakeAmountsTest is BaseTest {
        ...
        returns (uint256 settlementFee)
    {
        ...
        settlementFee = covenant.settlementFee(id, market.maturity - vm.getBlockTimestamp());
    }
    ...
    function _createPosition(uint256 positionUnits) internal {
        ...
        fillOffer(positionUnits, lender, offer);
    }
    ...
    function testBuyerAssetsToUnitsBuyerIsLender(
        ...
        offer.tick = tick;
        uint256 units = TakeAmountsLib.buyerAssetsToUnits(address(covenant), id, offer, targetBuyerAssets);
        deal(address(loanToken), lender, type(uint256).max);
        collateralize(market, borrower, units);
        offer.maker = borrower;
        offer.receiverIfMakerIsSeller = borrower;

        (uint256 buyerAssets,) = fillOffer(units, lender, offer);
    ...
    function testSellerAssetsToUnitsBuyerIsLender(
        ...
        offer.tick = tick;
        uint256 units = TakeAmountsLib.sellerAssetsToUnits(address(covenant), id, offer, targetSellerAssets);
        deal(address(loanToken), lender, type(uint256).max);
        collateralize(market, borrower, units);
        offer.maker = borrower;
        offer.receiverIfMakerIsSeller = borrower;

        (, uint256 sellerAssets) = fillOffer(units, lender, offer);
    ...
    function testBuyerAssetsToUnitsBuyerIsBorrower(
        ...
        (uint256 buyerAssets,) = fillOffer(units, borrower, offer);
    ...
    function testSellerAssetsToUnitsBuyerIsBorrower(
        ...
        (, uint256 sellerAssets) = fillOffer(units, borrower, offer);
    ...
    function testSnappedBuyerAssetsBuyerIsLender(
        ...
        (uint256 buyerAssets,) = fillOffer(targetUnits, lender, offer);
    ...
    function testSnappedBuyerAssetsBuyerIsBorrower(
        ...
        (uint256 buyerAssets,) = fillOffer(targetUnits, borrower, offer);
