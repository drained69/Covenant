...
import {DummyNotary} from "./helpers/DummyNotary.sol";
...
abstract contract BaseTest is Test {
    ...
    EcrecoverNotary internal ecrecoverNotary;
    EcrecoverAuthorizer internal ecrecoverAuthorizer;
    DummyNotary internal dummyNotary;

    bytes internal emptySig;

    function setUp() public virtual {
        // Existing test surface: `requireCompliance = false` (permissionless), matching the pre-Cleanverse
        // core behavior. A dedicated compliance-mode test suite covers the `true` branch.
        covenant = new Covenant(false, address(0));
        ecrecoverNotary = new EcrecoverNotary(address(covenant));
        ecrecoverAuthorizer = new EcrecoverAuthorizer(address(covenant));
        dummyNotary = new DummyNotary();

        covenant.setFeeSetter(address(this));
        covenant.setTickSpacingSetter(address(this));
        ...
        // Authorize the dummy notary (used by default in Covenant integration tests).
        vm.prank(borrower);
        covenant.setIsAuthorized(address(dummyNotary), true, borrower);
        vm.prank(lender);
        covenant.setIsAuthorized(address(dummyNotary), true, lender);
        vm.prank(otherBorrower);
        covenant.setIsAuthorized(address(dummyNotary), true, otherBorrower);
        vm.prank(otherLender);
        covenant.setIsAuthorized(address(dummyNotary), true, otherLender);

        // Authorize the ecrecover notary (used by notary-specific tests).
        vm.prank(borrower);
        covenant.setIsAuthorized(address(ecrecoverNotary), true, borrower);
        vm.prank(lender);
        covenant.setIsAuthorized(address(ecrecoverNotary), true, lender);
        vm.prank(otherBorrower);
        covenant.setIsAuthorized(address(ecrecoverNotary), true, otherBorrower);
        vm.prank(otherLender);
        covenant.setIsAuthorized(address(ecrecoverNotary), true, otherLender);
        ...
        loanToken.approve(address(covenant), type(uint256).max);
        collateralToken1.approve(address(covenant), type(uint256).max);
        collateralToken2.approve(address(covenant), type(uint256).max);
    }
    ...
    function collateralize(Market memory market, address _borrower, uint256 debt, uint256 collateralIndex) internal {
        ...
        ERC20(market.collateralParams[collateralIndex].token).approve(address(covenant), 0);
        ERC20(market.collateralParams[collateralIndex].token).approve(address(covenant), collateral);
        covenant.supplyCollateral(market, collateralIndex, collateral, _borrower);
        vm.stopPrank();
    }

    // Convenience wrapper for fillOffer with the dummy notary and no callbacks.
    function fillOffer(uint256 units, address taker, Offer memory offer) internal returns (uint256, uint256) {
        ...
        vm.prank(taker);
        return covenant.fillOffer(offer, hex"", units, taker, taker, address(0), hex"");
    }
    ...
    function setupOtherUsers(Market memory market, uint256 units) internal {
        ...
        fillOffer(units, otherBorrower, lenderOffer);
    }
    ...
    function createBadDebt(Market memory market) internal {
        ...
        badBorrowerOffer.notary = address(dummyNotary);
        ...
        covenant.setIsAuthorized(address(dummyNotary), true, badBorrower);
        vm.prank(badBorrower);
        covenant.setIsAuthorized(address(this), true, badBorrower);

        deal(market.collateralParams[0].token, address(this), 135);
        covenant.supplyCollateral(market, 0, 135, badBorrower);

        vm.prank(badBorrower);
        covenant.setIsAuthorized(address(this), false, badBorrower);

        deal(address(loanToken), unluckyLender, 100);

        fillOffer(100, unluckyLender, badBorrowerOffer);

        Oracle(market.collateralParams[0].oracle).setPrice(ORACLE_PRICE_SCALE / 4);
        covenant.seize(market, 0, 0, 0, badBorrower, false, address(this), address(0), "");

        // then empty the market (borrow side only).
        vm.prank(badBorrower);
        covenant.setIsAuthorized(address(this), true, badBorrower);
        deal(address(loanToken), address(this), covenant.debtOf(toId(market), badBorrower));
        covenant.repay(market, covenant.debtOf(toId(market), badBorrower), badBorrower, address(0), hex"");
        assertEq(covenant.debtOf(toId(market), badBorrower), 0, "debt");
    ...
    function setupMarket(Market memory market, uint256 units) internal {
        ...
        vm.prank(lender);
        covenant.fillOffer(borrowerOffer, hex"", units, lender, borrower, address(0), hex"");
    }
